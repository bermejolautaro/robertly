...
builder.Services.Configure<GoogleCredentialOptions>(
  builder.Configuration.GetSection(GoogleCredentialOptions.GoogleCredential)
);
...
static GoogleCredential GetGoogleCredential(
  GoogleCredentialOptions googleCredentialOptions
)
{
...
{
  var googleCredentialOptions =
    (serviceProvider.GetService<IOptions<GoogleCredentialOptions>>()?.Value)
