...
public class UserHelper
{
  ...
  public UserHelper(
    ...
    AppLogsRepository appLogs,
    FirebaseApp firebaseApp,
    IOptions<ConfigurationOptions> config
  ) =>
    (_userRepository, _appLogs, _firebaseApp, _testUserFirebaseUuid) = (
      userRepository,
      appLogs,
      firebaseApp,
      config.Value.TestUserFirebaseUuid
    );

  public async Task<Models.User?> GetUser(HttpRequest request)
  {
    if (!string.IsNullOrEmpty(_testUserFirebaseUuid))
    {
      return await _userRepository.GetUserByFirebaseUuidAsync(
        _testUserFirebaseUuid
      );
    }
    ...
    {
      await _appLogs.LogError(
        $"{e.GetType().Name} on {nameof(UserHelper)}.{nameof(UserHelper.GetUser)}",
        e
      );
      return null;
    }
